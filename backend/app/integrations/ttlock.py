...
async def delete_passcode_by_id(lock_id: int, keyboard_pwd_id: int) -> None:
    """
    NEW (28.7.26): מחיקה ישירה של קוד מהמנעול, לא קשור בהכרח להזמנה
    ב-DB — לניקוי קודים "יתומים" שהתגלו בעמוד ניהול המנעולים (קוד שקיים
    בפועל על המנעול אבל אין הזמנה פעילה שמצביעה עליו — למשל checkout
    cleanup שנכשל בשקט).
    """
    await ttlock_client.delete_passcode(lock_id, keyboard_pwd_id)
