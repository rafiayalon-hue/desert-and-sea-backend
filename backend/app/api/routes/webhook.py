...
def _extract_guests(header: dict, payload: dict, room: dict) -> tuple[int | None, int | None]:
    ...
    Walks the ENTIRE payload recursively (header, rooms, nested occupancy,
    anywhere) and grabs the first plausible adults / children / infants values
    by key name — case-insensitive, tolerant of camelCase and underscores.
    Infants are folded into children so a baby counts toward occupancy.
    Returns (None, None) only when truly nothing guest-like is present, so the
    caller keeps the existing/default value instead of zeroing real data.
    """
    adults = None
    children = None
    infants = None
    ...
    adult_keys = {
        "adults", "numadults", "adultcount", "adult", "adultscount",
        "numberofadults", "adultsnumber", "adultnum", "paxadults", "nadults",
    }
    child_keys = {
        "children", "numchildren", "childcount", "child", "childrennumber",
        "childrencount", "kids", "numberofchildren", "childnum", "paxchildren",
        "nchildren", "childs",
    }
    infant_keys = {
        "infants", "numinfants", "infant", "babies", "baby", "infantcount",
        "numberofinfants", "ninfants", "paxinfants",
    }

    def walk(o):
        nonlocal adults, children, infants
            ...
            for k, v in o.items():
                ...
                if isinstance(v, (int, float, str)):
                    try:
                        ...
                        num = None
                    if num is not None:
                        if kl in adult_keys and adults is None:
                            adults = num
                        elif kl in child_keys and children is None:
                            children = num
                        elif kl in infant_keys and infants is None:
                            infants = num
                walk(v)
    ...
    # search the whole payload; header and room are already inside payload but
    # pass them too in case the caller trimmed the structure.
    walk(payload)
    walk(header)
    walk(room)

    if infants is not None:
        children = (children or 0) + infants
