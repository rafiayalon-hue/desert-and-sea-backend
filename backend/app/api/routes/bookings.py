...
async def delete_booking(booking_id: int, db: AsyncSession = Depends(get_db)):
    ...
    מוחקת קודם רשומות message_logs תלויות (foreign key), אחר כך את ההזמנה.
    """
    booking = await db.get(Booking, booking_id)
    if not booking:
        raise HTTPException(status_code=404, detail="הזמנה לא נמצאה")
    ...
    # מוחקים בסדר מפורש דרך SQL גולמי: קודם התלויות (message_logs), אחר כך
    # ההזמנה עצמה. שימוש ב-db.delete(booking) של ה-ORM גרם ל-ForeignKeyViolation
    # כי ה-unit-of-work סידר מחדש את הפעולות והריץ את מחיקת ההזמנה לפני מחיקת
    # ה-logs. text() DELETE רץ מיידית ובסדר שנכתב, ולכן בטוח.
    # מוחקים את כל הטבלאות התלויות לפני ההזמנה עצמה, בסדר מפורש —
    # אחרת foreign key חוסם (message_logs ו-checkin_tokens שתיהן מצביעות
    # על bookings.id). SQL גולמי רץ מיידית ובסדר שנכתב, בניגוד ל-ORM
    # שסידר מחדש וגרם ל-ForeignKeyViolation.
    await db.execute(text("DELETE FROM message_logs WHERE booking_id = :bid"), {"bid": booking_id})
    await db.execute(text("DELETE FROM checkin_tokens WHERE booking_id = :bid"), {"bid": booking_id})
    await db.execute(text("DELETE FROM bookings WHERE id = :bid"), {"bid": booking_id})
    await db.commit()
    return {
        "status": "deleted",
...
class BookingUpdate(BaseModel):
    ...
    guest_email: Optional[str] = None  # NEW (16.7.26) — עריכת מייל מעמוד האורחים
...
async def update_booking(
    ...
    if data.guest_email is not None:
        booking.guest_email = data.guest_email.strip()
