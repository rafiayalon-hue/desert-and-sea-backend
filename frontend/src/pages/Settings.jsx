...
// NEW (16.7.26): שדה עם כפתור "העתק" — לפרטי בנק, כדי שאבישג תוכל
// להעתיק במהירות לצ'אט עם אורח בלי לסמן ידנית.
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        <button type="button" className="btn btn-secondary" style={{ fontSize: ".75rem", padding: "0 10px", whiteSpace: "nowrap" }} onClick={copy}>
          {copied ? "✓ הועתק" : "העתק"}
        </button>
      </div>
    </div>
...
export default function Settings() {
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      {/* NEW (16.7.26): פרטי בנק להעברה */}
      <Section title="🏦 פרטי בנק להעברה">
        <div style={{ fontSize: ".8rem", color: "var(--text-muted)", marginBottom: 12, padding: "8px 12px", background: "var(--sand-bg)", borderRadius: 8 }}>
          לאורחים שמבקשים לשלם בהעברה בנקאית — כפתור "העתק" ליד כל שדה לשליחה מהירה בצ'אט
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <CopyField label="בנק"            value={biz.bank_name}            onChange={v => updateBiz("bank_name", v)} />
          <CopyField label="סניף"           value={biz.bank_branch}          onChange={v => updateBiz("bank_branch", v)} />
          <CopyField label="מספר חשבון"      value={biz.bank_account_number}  onChange={v => updateBiz("bank_account_number", v)} />
          <CopyField label="על שם"          value={biz.bank_account_holder}  onChange={v => updateBiz("bank_account_holder", v)} />
        </div>
      </Section>
