...
// ===== כרטיס אורח מפורט =====
function GuestCard({ guest, onClose, onSaveNote, onSaveContact, onDeleteBooking }) {
            ...
            {guest.bookings.map((b, i) => (
              <div key={b.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button
                    title="מחיקת הזמנה"
                    onClick={() => onDeleteBooking(b.id, guest.key)}
                    style={{
...
export default function Guests() {
  ...
  // NEW (16.7.26): מחיקת הזמנה בודדת מתוך כרטיס האורח — למשל רשומות
  // "זבל" כמו הפסקת חשמל/שיפוץ, או הזמנת כפילות. פעולה בלתי הפיכה,
  // ולכן דורשת אישור מפורש לפני שליחה לשרת.
  async function deleteBooking(bookingId, guestKey) {
    ...
    try {
      ...
      invalidateGuestsCache();
      reload();
      // אם זו הייתה ההזמנה האחרונה של האורח, הכרטיס כבר לא רלוונטי — סוגרים אותו
      const guest = guests.find(g => g.key === guestKey);
      if (guest && guest.bookings.length <= 1) {
        setSelectedKey(null);
      }
    } catch (err) {
      console.error("[Guests] delete booking failed:", err);
