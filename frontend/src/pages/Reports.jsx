...
function calcMonthStats(confirmed, year, month) {
  ...
  let desertN = 0, seaN = 0, revenue = 0, count = 0, totalNights = 0;
  const sources = { airbnb: 0, direct: 0, website: 0, other: 0 };
  const sourceNights = { airbnb: 0, direct: 0, website: 0, other: 0 };

  // מחשב לפי לילות שנופלים בתוך החודש (תיקון 19.9.26 — קודם נספרה כל ההזמנה פעמיים אם חצתה חודש)
  const mStart = `${mStr}-01`;
  const mEnd = `${mStr}-${String(daysIn).padStart(2, "0")}`;
  for (const b of confirmed) {
    if (!b.checkin || !b.checkout) continue;
    if (b.checkin > mEnd || b.checkout <= mStart) continue;

    const rooms = b.rooms || [];
    const allNights = Math.round((new Date(b.checkout) - new Date(b.checkin)) / (1000 * 60 * 60 * 24));
    if (allNights <= 0) continue;
    const from = b.checkin > mStart ? b.checkin : mStart;
    const toExcl = b.checkout;
    const lastIncl = toExcl > mEnd ? mEnd : null;
    const nights = Math.round(((lastIncl ? new Date(lastIncl).getTime() + 86400000 : new Date(toExcl).getTime()) - new Date(from).getTime()) / 86400000);
    ...
    revenue += (b.total_price || 0) * nights / allNights;
    const startsHere = b.checkin >= mStart;
    if (startsHere) { count++; totalNights += allNights; }

    const src = (b.source || "").toLowerCase();
    const key = src === "airbnb" ? "airbnb" : src === "direct" ? "direct"
      : (src === "website" || src === "homepage") ? "website" : "other";
    if (startsHere) sources[key]++;
    sourceNights[key] += nights;
  }

  const avgNights = count > 0 ? Math.round((totalNights / count) * 10) / 10 : 0;

  return {
    mStr, daysIn, desertN, seaN, revenue: Math.round(revenue), count, sources, sourceNights, avgNights,
...
function YearOverYear({ confirmed }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <CompareRow label="לילות ממוצע להזמנה"  currVal={curr.avgNights}  prevVal={prev.avgNights}  format={v => v > 0 ? `${v}` : "—"} />
...
const CURR_COLOR = "#E07B4F";
const PREV_COLOR = "#6BAED6";
