...
function YearOverYear({ confirmed }) {
  ...
  function Delta({ curr, prev }) {
    ...
    return <span style={{ color, fontSize: ".75rem", fontWeight: 700 }}>{diff >= 0 ? "▲" : "▼"} {Math.abs(pct)}%</span>;
  }
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          <span style={{ color: "var(--text-muted)", minWidth: 80, textAlign: "left" }}>{display(prevVal)}</span>
          <span style={{ fontWeight: 700, minWidth: 80, textAlign: "left" }}>{display(currVal)}</span>
