...
export default function BookingDetail({ bookingId, navigate }) {
  ...
  const [localPhone,   setLocalPhone]   = useState(null); // אחרי שמירה
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="btn btn-secondary" style={{ flex: 1 }} onClick={() => setPreviewMsg(null)}>
                ביטול
              </button>
            </div>
          </div>
      ...
      <div className="detail-grid">
        ...
        <div>
              ...
              <span className="detail-value">
                {editingPhone ? (
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <input
                      className="input"
                      style={{ width: 140, padding: "4px 8px", fontSize: ".85rem" }}
          ...
          <Section title="הערות">
            ...
            <textarea className="textarea" placeholder="הערות פנימיות (לא נשלחות לאורח)..."
              value={internalNotes} onChange={e => setInternalNotes(e.target.value)} />
            <button className="btn btn-secondary btn-sm" style={{ marginTop: 8 }}
              onClick={() => alert("נשמר!")}>שמור הערות</button>
          </Section>
        </div>
            ...
            <Section title="תשלום">
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                {["credit_link", "cash", "bank"].map(pm => (
                  <button key={pm} className={`filter-btn ${paymentMethod === pm ? "active" : ""}`}
              ...
              {paymentMethod === "credit_link" && (
                <input className="input" placeholder="הדבק לינק ישראכרט 360..."
                  value={paymentLink} onChange={e => setPaymentLink(e.target.value)} style={{ marginBottom: 8 }} />
              )}
              <button className="btn btn-secondary btn-sm" onClick={() => alert("נשמר!")}>שמור</button>
            </Section>
          )}
