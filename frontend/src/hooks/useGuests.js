...
// NEW (16.7.26): מאפשר ל-Guests.jsx לאלץ רענון נתונים מהשרת אחרי שמירה
// מוצלחת (למשל טלפון/הערה) — בלי זה, ה-cache הישן (בזיכרון המודול) היה
// ממשיך להציג את הנתונים הישנים עד לרענון מלא של הדף (F5).
export function invalidateGuestsCache() {
  cache = null;
}
...
export function useGuests() {
  ...
  // NEW (16.7.26): מאפשר לרכיבים לבקש רענון נתונים אחרי מחיקה/שמירה,
  // בלי לחכות לרענון מלא של הדף. משתמשים יחד עם invalidateGuestsCache().
  function reload() {
    setLoading(true);
    setReloadTick(t => t + 1);
  }
